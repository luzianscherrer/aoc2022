...
function readInput(): string[] {
  const fileContent = readFileSync("day16.txt", "utf-8");
  const lines: string[] = [];
  fileContent.split(/\r?\n/).forEach((line) => {
...
function searchPath() {
  ...
  for (let key of distances.get(nodes[0].name)!.keys()) {
    ...
    queue.push({
      node: key,
      elapsed: elapsed,
      output: rates.get(key)! * (30 - elapsed),
      visited: [key],
    });
  }
  ...
  while (queue.length) {
    ...
    if (element!.output > maxOutput) {
      ...
      console.log(JSON.stringify(element!.visited), maxOutput);
    }
    for (let key of distances.get(element!.node)!.keys()) {
      ...
      queue.push({
        node: key,
        elapsed: elapsed,
        output: element!.output + rates.get(key)! * (30 - elapsed),
