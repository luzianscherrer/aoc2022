...
async function main() {
  ...
  for (const line of lines) {
    elements.push({ index: count++, value: 811589153 * +line });
  }
  ...
  for (let k = 0; k < 10; k++) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (element.value === 0) {
        // console.log(`skip ${element.value}`);
        continue;
      }
      ...
      // console.log(
      //   `move ${element.value} (from ${idx} to ${
      //     mod(idx + element.value, count - 1) == 0
      //       ? count - 1
      //       : mod(idx + element.value, count - 1)
      //   })`
      // );
